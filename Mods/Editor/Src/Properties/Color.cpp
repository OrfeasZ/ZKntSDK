...
bool Editor::SColorRGBProperty(const std::string& p_Id, ZEntityRef p_Entity, SPropertyData* p_Property, void* p_Data) {
    ...
    if (ImGuiCopyWidget(("SColorRGB_" + p_Id).c_str())) {
        ...
        CopyToClipboard(fmt::format("#{0:02X}{1:02X}{2:02X}", r, g, b));
    }

    return s_IsChanged;
}

bool Editor::SColorRGBAProperty(const std::string& p_Id, ZEntityRef p_Entity, SPropertyData* p_Property, void* p_Data) {
    bool s_IsChanged = false;
    ...
    if (ImGuiCopyWidget(("SColorRGBA_" + p_Id).c_str())) {
        ...
        CopyToClipboard(fmt::format("#{0:02X}{1:02X}{2:02X}{3:02X}", r, g, b, a));
    }
