...
bool Editor::ArrayProperty(
    ...
    if (s_IsTreeNodeOpen) {
        s_IsChanged = DrawArrayElements(p_Renderer, p_Id, p_Entity, p_Property, p_Data, s_ArrayType);
...
bool Editor::DrawArrayElements(
    ...
    for (; s_Iterator != s_End; s_Iterator = p_ArrayType->containerType.pVTab->next(p_Data, s_Iterator), ++s_Index) {
        ...
        s_IsAnyElementChanged |=
            DrawEntityPropertyValue(p_Renderer, s_ElementId, "", s_ElementTypeName, s_ElementTypeID, p_Entity, p_Property, s_Iterator);
