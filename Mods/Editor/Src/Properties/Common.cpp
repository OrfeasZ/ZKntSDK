...
void Editor::OnSetPropertyValue(ZEntityRef p_Entity, uint32_t p_PropertyId, const ZObjectRef& p_Value, std::optional<std::string> p_ClientId) {
    SDK()->Functions()->SetPropertyValue->Call(p_Entity.m_pObj, p_PropertyId, p_Value, true);
    m_Server.OnEntityPropertySet(p_Entity, p_PropertyId, std::move(p_ClientId));
}

void Editor::OnSignalEntityPin(ZEntityRef p_Entity, uint32_t p_PinId, bool p_Output, const ZObjectRef& p_Data) {
    if (p_Output) {
...
void Editor::OnSignalEntityPin(ZEntityRef p_Entity, const std::string& p_Pin, bool p_Output, const ZObjectRef& p_Data) {
    OnSignalEntityPin(p_Entity, Hash::Crc32(p_Pin.c_str(), p_Pin.size()), p_Output, p_Data);
}

void Editor::UnsupportedProperty(const std::string& p_Id, ZEntityRef p_Entity, SPropertyData* p_Property, void* p_Data) {
...
void Editor::ZEntityRefProperty(const std::string& p_Id, ZEntityRef p_Entity, SPropertyData* p_Property, void* p_Data) {
...
void Editor::TEntityRefProperty(const std::string& p_Id, ZEntityRef p_Entity, SPropertyData* p_Property, void* p_Data) {
...
void Editor::EntityRefProperty(const std::string& p_Id, ZEntityRef p_Entity) {
    ...
    if (ImGui::IsItemHovered()) {
        ...
        ImGui::GetWindowDrawList()->AddLine(
            ImVec2(s_Min.x, s_Max.y), ImVec2(s_Max.x, s_Max.y),
        ...
        if (!m_CachedEntityTree || !m_CachedEntityTree->m_Entity) {
            ...
            if (s_Iterator != m_CachedEntityTreeMap.end()) {
                ImGui::SetTooltip("%s", s_Iterator->second->m_Name.c_str());
            }
        }
    }
...
void Editor::ZRepositoryIDProperty(const std::string& p_Id, ZEntityRef p_Entity, SPropertyData* p_Property, void* p_Data) {
...
void Editor::ZGuidProperty(const std::string& p_Id, ZEntityRef p_Entity, SPropertyData* p_Property, void* p_Data) {
    if (auto s_Guid = reinterpret_cast<ZGuid*>(p_Data)) {
        ...
        if (ImGuiCopyWidget(("Guid_" + p_Id).c_str())) {
            CopyToClipboard(knt::util::ToLowerCase(s_GuidString.c_str()));
        }
    }
    else {
        constexpr auto s_TextColor = ImVec4(1.f, 1.f, 1.f, 0.5f);
...
bool Editor::ZCurveProperty(
    ...
    IType* s_ArrayTypeInfo = (*SDK()->Globals()->TypeRegistry)->GetTypeID("TArray<TFixedArray<float32>>")->GetTypeInfo();
    ...
    const bool s_IsTreeNodeOpen =
        ImGui::TreeNodeEx(fmt::format("{} ({} {})##{}", s_PropertyName, s_ArraySize, s_ArraySize == 1 ? "element" : "elements", p_Id).c_str());
    ...
    const auto s_Result = ImGui::ButtonEx(
        (std::string(ICON_MD_SHOW_CHART) + "##" + p_Id).c_str(), {m_CopyWidgetButtonSize, m_CopyWidgetButtonSize}, ImGuiButtonFlags_AlignTextBaseLine
    );
    ...
    if (s_IsTreeNodeOpen) {
        s_IsChanged = DrawArrayElements(p_Renderer, p_Id, p_Entity, p_Property, &s_Curve->data, s_ArrayType);
...
void Editor::PlotZCurve(const ZCurve* p_Curve) {
    ...
    for (int32_t i = 0; i < s_KeyCount - 1; ++i) {
        ...
        ImPlot::PlotLine(("Segment##" + std::to_string(i)).c_str(), s_Xs.data(), s_Ys.data(), (int32_t)s_Xs.size());
    }
    ...
    for (int32_t i = 0; i < s_KeyCount; ++i) {
        ...
        ImPlot::PlotScatter(("Key##" + std::to_string(i)).c_str(), &s_X, &s_Y, 1);
    }
}

float Editor::EvaluateZCurveSegment(const TFixedArray<float, 8>& p_Key, const TFixedArray<float, 8>& p_NextKey, float p_Time) {
    ...
    return p_Key[2] * s_T * s_T * s_T * s_T * s_T + p_Key[3] * s_T * s_T * s_T * s_T + p_Key[4] * s_T * s_T * s_T + p_Key[5] * s_T * s_T
           + p_Key[6] * s_T + p_Key[7];
}
...
std::string Editor::FormatZCurveForClipboard(ZCurve* p_Curve, void* p_Data, const IArrayType* p_ArrayType) {
    ...
    for (; s_Iterator != s_End; s_Iterator = p_ArrayType->containerType.pVTab->next(p_Data, s_Iterator), ++s_Index) {
        ...
        for (size_t i = 0; i < s_FixedArray->size(); ++i) {
            s_Result += fmt::format("\t\t{}{}", (*s_FixedArray)[i], (i + 1 < s_FixedArray->size()) ? "," : "");
