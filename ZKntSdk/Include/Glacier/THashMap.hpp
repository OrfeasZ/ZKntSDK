...
template<class T> class THashMapNode {
  public:
    int32_t m_iNext = 0;
    T m_data;
};

template<class T> struct SHashMapInfo {
...
template<typename T> class TDefaultHashMapPolicy<T*> {
  public:
    static uint64_t GetHashCode(const T* p_Key) {
...
template<class T> class THashMapIterator : public TIterator<THashMapNode<T>> {
  protected:
    THashMapIterator(const SHashMapInfo<T>* p_Info, uint32_t p_Bucket, THashMapNode<T>* p_Current)
        : TIterator<THashMapNode<T>>(p_Current), m_pInfo(p_Info), m_nBucket(p_Bucket) {}

    THashMapIterator(const SHashMapInfo<T>* p_Info) : TIterator<THashMapNode<T>>(nullptr), m_pInfo(p_Info), m_nBucket(UINT32_MAX) {}
  ...
  public:
    ...
    bool operator==(const THashMapIterator<T>& p_Other) const {
        return p_Other.m_pCurrent == this->m_pCurrent && p_Other.m_nBucket == m_nBucket && p_Other.m_pInfo == m_pInfo;
    }
  ...
  public:
    const SHashMapInfo<T>* m_pInfo = nullptr;
    uint32_t m_nBucket = 0;

    template<class K, class V, class H> friend class THashMap;
};

template<class TKeyType, class TValueType, class THashingPolicy = TDefaultHashMapPolicy<TKeyType>> class THashMap {
  public:
  ...
  public:
    ...
    ~THashMap() {
        ...
        IAllocator* p_Allocator = (*SDK()->Globals()->MemoryManager)->m_pNormalAllocator;
    ...
    void init(uint32_t p_BucketCount) {
        ...
        m_info.m_pNodes = static_cast<node_type*>(p_Allocator->AllocateAligned(sizeof(node_type) * p_BucketCount, alignof(node_type)));
    ...
    void ensureCapacity(uint32_t p_ElementCount) {
        if (m_info.m_nBuckets < p_ElementCount) {
            uint32_t p_NewBucketCount = static_cast<uint32_t>(m_info.m_nBuckets * 1.5f);
        ...
        for (uint32_t i = 0; i < m_info.m_nBuckets; ++i) {
            ...
            if (s_NodeIndex != UINT32_MAX) {
                return const_iterator(
                    reinterpret_cast<const_map_info_type>(&m_info), i,
    ...
    const_iterator find(const TKeyType& p_Key) const {
        if (auto* s_Node = findNode(p_Key)) {
            return const_iterator(
                reinterpret_cast<const_map_info_type>(&m_info), THashingPolicy::GetHashCode(p_Key) % m_info.m_nBuckets,
    ...
    iterator find(const TKeyType& p_Key) {
        if (auto* s_Node = findNode(p_Key)) {
            return iterator(&m_info, THashingPolicy::GetHashCode(p_Key) % m_info.m_nBuckets, s_Node);
        }
    ...
    iterator insert(const TKeyType& p_Key, const TValueType& p_Value) {
        ...
        if (m_iFree == UINT32_MAX) {
            ...
            s_NewNodeIndex = m_iFree;
            m_iFree = m_info.m_pNodes[s_NewNodeIndex].m_iNext;
        }

        node_type& s_NewNode = m_info.m_pNodes[s_NewNodeIndex];
        s_NewNode.m_data = TPair<TKeyType, TValueType>(p_Key, p_Value);
        s_NewNode.m_iNext = m_info.m_pBuckets[s_BucketIndex];
    ...
    void rehash(uint32_t p_NewBucketCount) {
        ...
        node_type* s_NewNodes = static_cast<node_type*>(p_Allocator->AllocateAligned(sizeof(node_type) * p_NewBucketCount, alignof(node_type)));
        ...
        if (m_info.m_nBuckets > 0 && m_info.m_pBuckets && m_info.m_pNodes) {
            for (uint32_t i = 0; i < m_info.m_nBuckets; ++i) {
                ...
                while (s_NodeIndex != UINT32_MAX) {
                    ...
                    const uint64_t s_Hash = THashingPolicy::GetHashCode(s_OldNode.m_data.first);
                    const uint32_t s_NewBucketIndex = static_cast<uint32_t>(s_Hash % p_NewBucketCount);

                    node_type& s_NewNode = s_NewNodes[s_NewSize];
                    s_NewNode.m_data = s_OldNode.m_data;
                    s_NewNode.m_iNext = s_NewBuckets[s_NewBucketIndex];
                    s_NewBuckets[s_NewBucketIndex] = s_NewSize++;

                    s_NodeIndex = s_OldNode.m_iNext;
                }
            }
            ...
            for (uint32_t i = 0; i < m_info.m_nBuckets; ++i) {
                ...
                while (s_NodeIndex != UINT32_MAX) {
                    ...
                    s_Node.m_data.second.~TValueType();
                    s_Node.m_data.first.~TKeyType();
    ...
    bool erase(const TKeyType& p_Key) {
        ...
        while (s_NodeIndex != UINT32_MAX) {
            ...
            if (s_pCurrentNode->m_data.first == p_Key) {
                eraseNode(s_BucketIndex, s_pCurrentNode);

                return true;
            }

            s_NodeIndex = s_pCurrentNode->m_iNext;
        }
    ...
    void eraseNode(uint32_t p_BucketIndex, node_type* p_TargetNode) {
        ...
        while (s_NodeIndex != UINT32_MAX) {
            ...
            if (s_pCurrentNode == p_TargetNode) {
                if (s_pPrevNode) {
                    s_pPrevNode->m_iNext = s_pCurrentNode->m_iNext;
                }
                else {
                    *s_pBucket = s_pCurrentNode->m_iNext;
                }

                s_pCurrentNode->m_data.second.~TValueType();
                s_pCurrentNode->m_data.first.~TKeyType();

                s_pCurrentNode->m_iNext = m_iFree;
                m_iFree = s_NodeIndex;
                --m_nSize;
