...
template<typename T> class TDefaultHashSetPolicy<T*> {
  public:
    static uint64_t GetHashCode(const T* p_Key) {
...
template<typename T> class THashSetNode {
  public:
    uint32_t m_iNext;
    T m_data;
};

template<typename T> struct SHashSetInfo {
    bool m_bFixed = false;
...
template<class T> class THashSetIterator : public TIterator<THashSetNode<T>> {
  protected:
    THashSetIterator(const SHashSetInfo<T>* p_Info, uint32_t p_Bucket, THashSetNode<T>* p_Current)
        : TIterator<THashSetNode<T>>(p_Current), m_pInfo(p_Info), m_nBucket(p_Bucket) {}

    THashSetIterator(const SHashSetInfo<T>* p_Info) : TIterator<THashSetNode<T>>(nullptr), m_pInfo(p_Info), m_nBucket(UINT32_MAX) {}
  ...
  public:
    ...
    template<class T, class H> friend class THashSet;
};

template<class T, class THashingPolicy = TDefaultHashSetPolicy<T>> class THashSet {
  public:
        ...
        for (uint32_t i = 0; i < m_info.m_nBuckets; ++i) {
            ...
            if (s_NodeIndex != UINT32_MAX) {
                return const_iterator(
                    reinterpret_cast<const_set_info_type>(&m_info), i, reinterpret_cast<THashSetNode<const T>*>(&m_info.m_pNodes[s_NodeIndex])
                );
            }
        }
    ...
    const_iterator find(const T& p_Key) const {
        if (auto* s_Node = findNode(p_Key)) {
            return const_iterator(
                reinterpret_cast<const_set_info_type>(&m_info), THashingPolicy::GetHashCode(p_Key) % m_info.m_nBuckets,
