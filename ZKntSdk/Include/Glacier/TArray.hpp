...
template<class T> class TArray {
  public:
    ...
    TArray(const std::vector<T>& p_Other) : m_pBegin(nullptr), m_pEnd(nullptr), m_pAllocationEnd(nullptr) {
        if (p_Other.size() == 0) {
            return;
        }
    ...
    TArray(const TArray<T>& p_Other) : m_pBegin(nullptr), m_pEnd(nullptr), m_pAllocationEnd(nullptr) {
        if (p_Other.size() == 0) {
            return;
        }
    ...
    ~TArray() {
        ...
        if (!hasInlineFlag()) {
            (*SDK()->Globals()->MemoryManager)->m_pNormalAllocator->Free(m_pBegin);
        }
    }
    ...
    void push_back(const T& p_Value) {
        ...
        if (m_pEnd < m_pAllocationEnd) {
            ...
            const auto s_NewBegin =
                static_cast<T*>((*SDK()->Globals()->MemoryManager)->m_pNormalAllocator->AllocateAligned(sizeof(T) * s_NewCapacity, alignof(T)));
...
template<typename TElement, size_t TCapacity> class ZFixedArrayData {
  public:
    TElement m_pStart[TCapacity];
};

template<typename TElement> class ZArrayRefData {
  public:
    TElement* m_pStart;
    TElement* m_pEnd;
};

template<typename TElement, int TCapacity> class ZMaxArrayData {
  public:
    union {
...
template<typename TElement, typename TStorage> class TFixedArrayBase : public TStorage {
  public:
    size_t size() const {
...
template<typename TElement, size_t TCapacity> class TFixedArray : public TFixedArrayBase<TElement, ZFixedArrayData<TElement, TCapacity>> {};

template<typename TElement> class TArrayRef : public TFixedArrayBase<TElement, ZArrayRefData<TElement>> {
  public:
    TArrayRef() = default;
