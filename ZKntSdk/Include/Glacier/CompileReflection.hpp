...
namespace detail {
    ...
    template<> constexpr std::string_view ComputeTypeName<void>() {
        return "void";
    }
    ...
    constexpr std::size_t WrappedTypeNameSuffixLength() {
        return WrappedTypeName<TypeNameProber>().length() - WrappedTypeNamePrefixLength() - ComputeTypeName<TypeNameProber>().length();
    }
    ...
    template<std::size_t N> struct ZHMTypeNameData {
    ...
    template<std::size_t N> constexpr auto ComputeZHMTypeName(std::string_view p_TypeName) noexcept {
        ...
        for (std::size_t i = 0; i < N; ++i) {
            ...
            if (p_TypeName[i] == 's' && i + s_StructPrefix.size() < N && p_TypeName.substr(i, s_StructPrefix.size()) == s_StructPrefix) {
    ...
    template<StringLiteral Path> constexpr ZRuntimeResourceID IOIPathToRuntimeResourceID() {
 ...
 */
template<class T> inline constexpr auto TypeName = detail::ComputeTypeName<T>();
 ...
 */
template<class T> inline constexpr uint32_t ZHMTypeId = Hash::Crc32(ZHMTypeName<T>.data(), ZHMTypeName<T>.size());
