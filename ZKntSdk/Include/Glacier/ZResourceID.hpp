...
class ZRuntimeResourceID {
  public:
    constexpr ZRuntimeResourceID() : m_IDHigh(-1), m_IDLow(-1) {}

    constexpr ZRuntimeResourceID(uint64_t p_ID) : m_IDHigh(p_ID >> 32), m_IDLow(p_ID & 0xFFFFFFFF) {}

    constexpr ZRuntimeResourceID(uint32_t p_IDHigh, uint32_t p_IDLow) : m_IDHigh(p_IDHigh), m_IDLow(p_IDLow) {}
...
template<> struct std::hash<ZRuntimeResourceID> {
...
template<> struct fmt::formatter<ZRuntimeResourceID> {
...
template<> struct fmt::formatter<ZResourceID> {
    constexpr auto parse(format_parse_context& ctx) -> format_parse_context::iterator {
        return ctx.begin();
    }
