...
class ZSparseBitArray {
  public:
    class Iterator {
      public:
        Iterator(const ZSparseBitArray* p_Mask) : m_BitMask(p_Mask) {
            m_nIndex = p_Mask->m_nSize > 0 ? p_Mask->NextBit(-1) : p_Mask->m_nSize;
        }

        Iterator(const ZSparseBitArray* p_Mask, int32 p_Index) : m_BitMask(p_Mask) {
    ...
    int32 SkipStencil(int32 p_Index) const {
        ...
        return static_cast<int32>((static_cast<int64>(s_BlockIdx) * BITS_PER_BLOCK + s_FoundBitPos) * STENCIL_GRANULARITY);
    }
