...
class IPageAllocator {
  public:
    enum class EMemType { EMEMTYPE_MAIN = 0, EMEMTYPE_GPU = 1, EMEMTYPE_GPU_WC = 2, EMEMTYPE_FLEXIBLE = 3, EMEMTYPE_GEOMETRY = 4 };
  ...
  public:
    ...
    static void Commit(void* p_Address, uint32_t p_Offset, uint32_t p_Size, EVirtualMemoryPageSize p_PageSize) {
        ...
        while (s_Address < s_EndAddress) {
            s_Allocator->Commit(s_Address, s_PageSize, IPageAllocator::EMemType::EMEMTYPE_MAIN);
    ...
    static void Release(void* p_Address, uint32_t p_Offset, uint32_t p_Size, EVirtualMemoryPageSize p_PageSize) {
        GetAllocator(p_PageSize)->Decommit(static_cast<uint8_t*>(p_Address) + p_Offset, p_Size);
    }

  private:
    static IPageAllocator* GetAllocator(EVirtualMemoryPageSize p_PageSize) {
        auto* s_MemoryManager = *SDK()->Globals()->MemoryManager;

        return p_PageSize == EVirtualMemoryPageSize::Unknown0 ? s_MemoryManager->m_pPageAllocator2 : s_MemoryManager->m_pPageAllocator1;
    }
};
