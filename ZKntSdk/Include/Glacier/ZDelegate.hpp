...
template<class T> class ZDelegate;

template<class ReturnType, class... Args> class ZDelegate<ReturnType(Args...)> {
  public:
    ZDelegate() : m_mfp(nullptr), m_pad(nullptr), pStaticFunc(nullptr), pInst(nullptr) {}
  ...
  protected:
    ...
    ZDelegate(MemberCallback_t p_MemberCb, StaticCallback_t p_StaticCb, void* p_MemberInstance)
        : m_mfp(p_MemberCb), m_pad(nullptr), pStaticFunc(p_StaticCb), pInst(p_MemberInstance) {}
...
template<class InstanceType, class T> class ZMemberDelegate;
...
template<class ReturnType, class... Args> class ZStaticDelegate<ReturnType(Args...)> : public ZDelegate<ReturnType(Args...)> {
  public:
