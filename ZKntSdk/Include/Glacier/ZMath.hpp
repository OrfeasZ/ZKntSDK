...
class SVector3 {
  public:
    ...
    static SVector3 CrossProduct(const SVector3& p_Vector1, const SVector3& p_Vector2) {
        return SVector3(
            p_Vector1.y * p_Vector2.z - p_Vector1.z * p_Vector2.y, p_Vector1.z * p_Vector2.x - p_Vector1.x * p_Vector2.z,
...
class SMatrix43 {
  public:
    SMatrix43() : XAxis(1.f, 0.f, 0.f), YAxis(0.f, 1.f, 0.f), ZAxis(0.f, 0.f, 1.f), Trans(0.f, 0.f, 0.f) {}
...
template<> struct fmt::formatter<float4> {
...
struct alignas(16) SMatrix {
    ...
    SMatrix(DirectX::XMMATRIX p_Other) : XAxis(p_Other.r[0]), YAxis(p_Other.r[1]), ZAxis(p_Other.r[2]), Trans(p_Other.r[3]) {}
    ...
    DecomposedTransform Decompose() const {
        ...
        return {s_Trans, Quat(float4(s_RotQuat)), s_Scale};
    }
    ...
    static SMatrix ScaleTranslate(const float4& p_Scale, const float4& p_Translate) {
        return SMatrix(
            {p_Scale.x, 0.f, 0.f, 0.f}, {0.f, p_Scale.y, 0.f, 0.f}, {0.f, 0.f, p_Scale.z, 0.f}, {p_Translate.x, p_Translate.y, p_Translate.z, 1.f}
        );
    }
    ...
    static SMatrix RotationAxisAngle(const float4& p_Axis, const float p_Angle) {
        ...
        s_Result.XAxis = float4(c + (1.f - c) * x * x, (1.f - c) * y * x - s * z, (1.f - c) * z * x + s * y, 0.f);

        s_Result.YAxis = float4((1.f - c) * x * y + s * z, c + (1.f - c) * y * y, (1.f - c) * z * y - s * x, 0.f);

        s_Result.ZAxis = float4((1.f - c) * x * z - s * y, (1.f - c) * y * z + s * x, c + (1.f - c) * z * z, 0.f);
...
template<> struct fmt::formatter<SMatrix> {
    constexpr auto parse(format_parse_context& ctx) -> format_parse_context::iterator {
        return ctx.begin();
    }
