...
class ZObjectPool {
  public:
        ...
        while (true) {
            ...
            if (static_cast<uint32_t>(s_FreeListStart) == UINT32_MAX) {
                ...
                if (_InterlockedExchangeAdd64(&m_nFreeListStart, 0) == s_FreeListStart) {
                    ...
                    const uint32_t s_LastIndex = s_FirstIndex + m_nObjectDelta * (m_nGrowCount - 1);

                    for (uint32_t s_Index = s_FirstIndex; s_Index < s_LastIndex; s_Index += m_nObjectDelta) {
                    ...
                    while (true) {
                        const int64_t s_OldFreeListStart = _InterlockedExchangeAdd64(&m_nFreeListStart, 0);

                        m_pData[s_LastIndex] = static_cast<uint32_t>(s_OldFreeListStart);

                        const int64_t s_NewFreeListStart =
                            static_cast<int64_t>((static_cast<uint64_t>(s_OldFreeListStart) + 0x100000000ULL) & 0xFFFFFFFF00000000ULL) | s_FirstIndex;

                        if (_InterlockedCompareExchange64(&m_nFreeListStart, s_NewFreeListStart, s_OldFreeListStart) == s_OldFreeListStart) {
                            break;
                        }
                    }
                }
            ...
            const int64_t s_NewFreeListStart =
                static_cast<int64_t>((static_cast<uint64_t>(s_FreeListStart) + 0x100000000ULL) & 0xFFFFFFFF00000000ULL) | s_NextIndex;

            if (_InterlockedCompareExchange64(&m_nFreeListStart, s_NewFreeListStart, s_FreeListStart) == s_FreeListStart) {
    ...
    void Free(void* p_Object) {
        const uint32_t s_Index = static_cast<uint32_t>((reinterpret_cast<uintptr_t>(p_Object) - reinterpret_cast<uintptr_t>(m_pData)) >> 2);
        ...
        while (true) {
            ...
            *static_cast<uint32_t*>(p_Object) = static_cast<uint32_t>(s_FreeListStart);
            ...
            const int64_t s_NewFreeListStart =
                static_cast<int64_t>((static_cast<uint64_t>(s_FreeListStart) + 0x100000000ULL) & 0xFFFFFFFF00000000ULL) | s_Index;
    ...
    bool Contains(const void* p_Object) const {
        return m_pData && reinterpret_cast<uintptr_t>(p_Object) - reinterpret_cast<uintptr_t>(m_Buffer.m_pData) < m_Buffer.m_nActualSize;
    }
