...
namespace knt::util {
    ...
    inline bool InputWithAutocomplete(
        ...
        const bool s_IsEnterPressed = ImGui::InputText(s_InputId.c_str(), p_InputBuffer, p_BufferSize, ImGuiInputTextFlags_EnterReturnsTrue);
        ...
        if (s_IsOpen) {
            for (auto& s_Item : p_Collection) {
                ...
                if (!knt::util::FindSubstring(s_OptionName, p_InputBuffer, p_IsCaseSensitiveSearch)) {
                    continue;
                }
    ...
    static bool InputWithAutocomplete(
        ...
        const bool s_IsEnterPressed = ImGui::InputText(s_InputId.c_str(), &p_InputBuffer, ImGuiInputTextFlags_EnterReturnsTrue);
        const bool s_IsActive = ImGui::IsItemActive();
