    ...
    template<typename TVertex> class CustomPrimitiveBatch {
      ...
      public:
        CustomPrimitiveBatch(
            ID3D12Device* p_Device, FlushBatchCallback p_FlushBatchCallback, size_t p_MaxIndices = m_DefaultBatchSize * 3,
            ...
            if ((uint64_t(p_MaxVertices) * uint64_t(p_VertexSize))
                > uint64_t(D3D12_REQ_RESOURCE_SIZE_IN_MEGABYTES_EXPRESSION_A_TERM * 1024u * 1024u)) {
        ...
        void Draw(
            D3D_PRIMITIVE_TOPOLOGY p_Topology, bool p_IsIndexed, uint16_t const* p_Indices, size_t p_IndexCount, size_t p_VertexCount,
            ...
            if (p_Topology != m_CurrentTopology || p_IsIndexed != m_CurrentlyIndexed || s_WrapIndexBuffer || s_WrapVertexBuffer) {
                FlushBatch();
            }
        ...
        void Draw(D3D_PRIMITIVE_TOPOLOGY p_Topology, TVertex const* p_Vertices, size_t p_VertexCount) {
