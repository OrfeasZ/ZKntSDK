...
namespace zknt::rendering {
    DebugEffect::DebugEffect(
        ...
        if (!CompileShaderFromString(s_DebugVertexShader, "main", "vs_5_0", &s_VertexShaderBlob.m_Ref)) {
            return;
        }

        if (!CompileShaderFromString(s_DebugPixelShader, "main", "ps_5_0", &s_PixelShaderBlob.m_Ref)) {
            return;
        }
        ...
        DirectX::EffectPipelineStateDescription s_EffectPipelineStateDescription(
            p_InputLayoutDesc, DirectX::CommonStates::Opaque, DirectX::CommonStates::DepthReverseZ, DirectX::CommonStates::CullNone,
            s_RenderTargetState
        );
    ...
    void DebugEffect::CreateRootSignature(ID3D12Device* p_Device) {
    ...
    void DebugEffect::CreateTexture(ID3D12Device* p_Device, ID3D12GraphicsCommandList* p_CommandList, ID3D12CommandQueue* p_CommandQueue) {
        ...
        auto s_TextureDesc = CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_R8G8B8A8_UNORM, s_nTextureWidth, s_nTextureHeight, 1, 1);

        CD3DX12_HEAP_PROPERTIES s_DefaultHeap(D3D12_HEAP_TYPE_DEFAULT);

        HRESULT s_Result = p_Device->CreateCommittedResource(
            &s_DefaultHeap, D3D12_HEAP_FLAG_NONE, &s_TextureDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&m_pTextureResource)
        );
        ...
        s_Upload.Transition(m_pTextureResource, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
    ...
    void
    DebugEffect::CreateShaderResourceView(ID3D12Device* p_Device, const CD3DX12_RESOURCE_DESC& p_TextureDesc, ID3D12Resource* p_TextureResource) {
    ...
    void DebugEffect::CreateSampler(ID3D12Device* p_Device) {
    ...
    void DebugEffect::Apply(ID3D12Device* p_Device, ID3D12GraphicsCommandList* p_CommandList) {
