...
struct DetourTrampoline {
    DetourTrampoline(uintptr_t p_Address) : FunctionAddress(p_Address) {
        // NOP
  ...
  public:
    ...
    CreateTrampoline(uintptr_t p_TargetAddress, void* p_AdditionalInstructions = nullptr, size_t p_AdditionalInstructionsSize = 0) {
        if (g_Trampolines == nullptr) {
            ...
            auto s_AllocAddress = ALIGN_TO(
                reinterpret_cast<uintptr_t>(GetModuleHandleA(nullptr)) + zknt::ModSDK::GetInstance()->GetImageSize(),
                s_SysInfo.dwAllocationGranularity
            );
            ...
            while (s_QueryResult != 0
                   && (s_MemInfo.State != MEM_FREE || reinterpret_cast<uintptr_t>(s_MemInfo.BaseAddress) + s_MemInfo.RegionSize < s_AllocEnd)) {
                s_AllocAddress =
                    ALIGN_TO(reinterpret_cast<uintptr_t>(s_MemInfo.BaseAddress) + s_MemInfo.RegionSize, s_SysInfo.dwAllocationGranularity);
                s_AllocEnd = s_AllocAddress + sizeof(DetourTrampoline) * MAX_TRAMPOLINES;

                Logger::Trace(
                    "Memory segment at {} is not suitable. Trying next at {}.", fmt::ptr(s_MemInfo.BaseAddress),
                    fmt::ptr(reinterpret_cast<void*>(s_AllocAddress))
                );
            ...
            g_Trampolines = static_cast<DetourTrampoline*>(VirtualAlloc(
                reinterpret_cast<void*>(s_AllocAddress), sizeof(DetourTrampoline) * MAX_TRAMPOLINES, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE
            ));
...
template<class T> class HookImpl;

template<class ReturnType, class... Args> class HookImpl<ReturnType(Args...)> : public Hook<ReturnType(Args...)> {
  protected:
    HookImpl(const char* p_HookName, void* p_Target, typename Hook<ReturnType(Args...)>::OriginalFunc_t p_Detour) : m_Target(p_Target) {
        ...
        const auto s_InitResult = MH_Initialize();
        if (s_InitResult != MH_OK && s_InitResult != MH_ERROR_ALREADY_INITIALIZED) {
            Fail();
            Logger::Error("Could not initialize MinHook for hook '{}'. Error code: {}.", p_HookName, static_cast<int>(s_InitResult));
            return;
        }
...
template<class T> class PatternHook;

template<class ReturnType, class... Args> class PatternHook<ReturnType(Args...)> final : public HookImpl<ReturnType(Args...)> {
  public:
  ...
  private:
    void* GetTarget(const char* p_Pattern, const char* p_Mask) const {
        ...
        return reinterpret_cast<void*>(Util::ProcessUtils::SearchPattern(
            zknt::ModSDK::GetInstance()->GetModuleBase(), zknt::ModSDK::GetInstance()->GetSizeOfCode(), s_Pattern, p_Mask
        ));
    }
};

template<class T> class PatternCallHook;

template<class ReturnType, class... Args> class PatternCallHook<ReturnType(Args...)> final : public HookImpl<ReturnType(Args...)> {
  public:
    ...
    void Remove() override {
        ...
        for (auto* s_Detour : this->m_Detours) {
            delete s_Detour;
        }
  ...
  private:
    typename Hook<ReturnType(Args...)>::OriginalFunc_t InstallDetourAndGetOriginal(
        const char* p_HookName, const char* p_Pattern, const char* p_Mask, typename Hook<ReturnType(Args...)>::OriginalFunc_t p_Detour
    ) {
        const auto* s_Pattern = reinterpret_cast<const uint8_t*>(p_Pattern);
        m_Target = Util::ProcessUtils::SearchPattern(
            zknt::ModSDK::GetInstance()->GetModuleBase(), zknt::ModSDK::GetInstance()->GetSizeOfCode(), s_Pattern, p_Mask
        );
        ...
        if (s_Distance >= INT32_MAX || s_Distance <= INT32_MIN) {
            Logger::Trace(
                ...
                fmt::ptr(reinterpret_cast<void*>(m_Target + 5)), s_Distance
            );
            ...
            if (s_Distance >= INT32_MAX || s_Distance <= INT32_MIN) {
                Logger::Error(
                    ...
                    fmt::ptr(reinterpret_cast<void*>(s_TrampolineAddress)), fmt::ptr(reinterpret_cast<void*>(m_Target + 5)), s_Distance
                );
                return nullptr;
            }
        }
...
template<class T> class PatternRelativeCallHook;

template<class ReturnType, class... Args> class PatternRelativeCallHook<ReturnType(Args...)> final : public HookImpl<ReturnType(Args...)> {
  public:
...
template<class T> class PatternVtableHook;

template<class ReturnType, class... Args> class PatternVtableHook<ReturnType(Args...)> final : public HookImpl<ReturnType(Args...)> {
  public:
    PatternVtableHook(
        const char* p_HookName, const char* p_Pattern, const char* p_Mask, size_t p_VtableIndex,
        typename Hook<ReturnType(Args...)>::OriginalFunc_t p_Detour
    )
  ...
  private:
    void* GetTarget(const char* p_HookName, const char* p_Pattern, const char* p_Mask, size_t p_VtableIndex) const {
        ...
        if (s_Target != 0 && *reinterpret_cast<uint8_t*>(s_Target) != 0x48) {
            Logger::Error(
                "Expected a rex prefix for vtable hook '{}' at address {} but instead got 0x{:02X}.", p_HookName,
                fmt::ptr(reinterpret_cast<void*>(s_Target)), *reinterpret_cast<uint8_t*>(s_Target)
            );
            return nullptr;
        }
...
template<class T> class ModuleHook;

template<class ReturnType, class... Args> class ModuleHook<ReturnType(Args...)> final : public HookImpl<ReturnType(Args...)> {
  public:
    ModuleHook(
        const char* p_HookName, const char* p_ModuleName, const char* p_FunctionName, typename Hook<ReturnType(Args...)>::OriginalFunc_t p_Detour
    )
...
#define PATTERN_HOOK(Pattern, Mask, HookName, HookType)                                                        \
    this->HookName = new PatternHook<HookType>(                                                                \
        #HookName, Pattern, Mask, (typename Hook<HookType>::OriginalFunc_t)[]<class... Args>(Args... p_Args) { \
            static auto* s_Hook = SDK()->Hooks()->HookName;                                                    \
            if (auto s_Ticket = zknt::g_HookGate.TryEnter(); s_Ticket) {                                       \
                return s_Hook->Call(p_Args...);                                                                \
            }                                                                                                  \
            return s_Hook->CallOriginal(p_Args...);                                                            \
        }                                                                                                      \
    )

#define PATTERN_VTABLE_HOOK(Pattern, Mask, VtableIndex, HookName, HookType)                                                 \
    this->HookName = new PatternVtableHook<HookType>(                                                                       \
        #HookName, Pattern, Mask, VtableIndex, (typename Hook<HookType>::OriginalFunc_t)[]<class... Args>(Args... p_Args) { \
            static auto* s_Hook = SDK()->Hooks()->HookName;                                                                 \
            if (auto s_Ticket = zknt::g_HookGate.TryEnter(); s_Ticket) {                                                    \
                return s_Hook->Call(p_Args...);                                                                             \
            }                                                                                                               \
...
#define PATTERN_CALL_HOOK(Pattern, Mask, HookName, HookType)                                                   \
    this->HookName = new PatternCallHook<HookType>(                                                            \
        #HookName, Pattern, Mask, (typename Hook<HookType>::OriginalFunc_t)[]<class... Args>(Args... p_Args) { \
            static auto* s_Hook = SDK()->Hooks()->HookName;                                                    \
            if (auto s_Ticket = zknt::g_HookGate.TryEnter(); s_Ticket) {                                       \
                return s_Hook->Call(p_Args...);                                                                \
...
#define PATTERN_RELATIVE_CALL_HOOK(Pattern, Mask, HookName, HookType)                                          \
    this->HookName = new PatternRelativeCallHook<HookType>(                                                    \
        #HookName, Pattern, Mask, (typename Hook<HookType>::OriginalFunc_t)[]<class... Args>(Args... p_Args) { \
            static auto* s_Hook = SDK()->Hooks()->HookName;                                                    \
            if (auto s_Ticket = zknt::g_HookGate.TryEnter(); s_Ticket) {                                       \
                return s_Hook->Call(p_Args...);                                                                \
...
#define MODULE_HOOK(ModuleName, FunctionName, HookName, HookType)                                                         \
    this->HookName = new ModuleHook<HookType>(                                                                            \
